...
class Pose:
    ...
    def draw(self, img,is_save = False,show_draw = True):
        assert self.keypoints.shape == (Pose.num_kpts, 2)
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):   # 将躯干某个连接的单位向量映射到paf对应的通道
            ...
            if global_kpt_a_id != -1:  # 分配了当前关节点
                x_a, y_a = self.keypoints[kpt_a_id]  # 当前关节点在原图像上的坐标
                x_a, y_a = int(x_a),int(y_a)
                if show_draw:
                    cv2.circle(img, (x_a, y_a), 3, Pose.color, -1)   # 原图画圆
            ...
            if global_kpt_b_id != -1:  # 分配了当前关节点
                ...
                if show_draw:
                    cv2.circle(img, (x_b, y_b), 3, Pose.color, -1)    # 原图画圆
                # **************************************
            ...
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:  # 起点和终点均分配
                if show_draw:
                    cv2.line(img, (x_a, y_a), (x_b, y_b), Pose.color, 2)  # 画连接起点和终点的直线
                # **************************************
